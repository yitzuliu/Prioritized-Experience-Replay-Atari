...
BATCH_SIZE = 128  # Reduced batch size to save memory (減小批次大小以節省記憶體)
MEMORY_CAPACITY = 100000  # Reduced memory capacity (減小記憶容量)
...
# Logger settings
# 日誌記錄器設置
MIN_WRITE_INTERVAL = 60  # Minimum seconds between writes (寫入之間的最小秒數)
MAX_WRITE_INTERVAL = 600  # Maximum seconds between writes (寫入之間的最大秒數)
DEFAULT_WRITE_INTERVAL = 300  # Default initial write interval (默認初始寫入間隔)
MAX_BUFFER_SIZE = 500  # Maximum buffer size for in-memory storage (內存存儲的最大緩衝區大小)
MIN_BUFFER_SIZE = 50  # Minimum buffer size for in-memory storage (內存存儲的最小緩衝區大小)
DEFAULT_BUFFER_SIZE = 100  # Default initial buffer size (默認初始緩衝區大小)
SEGMENT_SIZE = 1000  # Maximum episodes per segment file (每個段文件的最大回合數)
ASYNC_QUEUE_SIZE = 100  # Size of async writer queue (異步寫入隊列大小)
TRAINING_STEP_LOG_FREQUENCY = 100  # How often to log training steps (多久記錄一次訓練步驟)
MAX_TRAINING_STEP_ENTRIES = 1000  # Maximum number of training step entries to keep (訓練步驟條目的最大保存數量)
RECOVERY_SAVE_INTERVAL = 1800  # Seconds between recovery info saves (恢復信息保存之間的秒數，設為半小時)